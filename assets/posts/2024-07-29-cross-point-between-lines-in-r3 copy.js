...
function init() {
  ...
  const intersectionLine = r2.clone().sub(r1.clone());
  const cp = r1.add(intersectionLine.clone().multiplyScalar(0.5));
  geometry = new THREE.BoxGeometry(0.1, 0.1, 0.1);
  ...
  let theta = direction1.clone().normalize().dot(intersectionLine.clone().normalize());

  //theta = theta / (direction1.clone().length() * intersectionLine.clone().length());
  console.log(theta);
  console.log(Math.acos(theta) * 180 / Math.PI);

  theta = direction2.clone().normalize().dot(intersectionLine.clone().normalize());
  //theta = theta / (direction2.clone().length() * intersectionLine.clone().length());
  console.log(theta);
  console.log(Math.acos(theta) * 180 / Math.PI);

  theta = new THREE.Vector3(1, 0, 0).dot(new THREE.Vector3(0, 1, 0));
  console.log(theta);


  createLine(new THREE.Vector3(0, 0, 0), intersectionLine.clone().normalize(), 1, scene, 0xffff00);
  createLine(new THREE.Vector3(0, 0, 0), direction1.clone().normalize(), 1, scene, 0xffff00);
  //createLine(origin1, direction1, s, scene, 0xff0000);

  //const d1 = new THREE.Vector3(2, 2, 2);

  // const r = direction1.clone().normalize();
  // const s = direction2.clone().normalize();
  // const matA = THREE.Matrix3(
